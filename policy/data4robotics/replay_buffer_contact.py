...
class RobobufReplayBuffer(Dataset):
    def __init__(
        ...
        # Load contact anchor normalization stats if available
        contact_norm_file = os.path.join(os.path.dirname(buffer_path), "contact_norm.json")
        self._contact_loc = None
        self._contact_scale = None
        if os.path.exists(contact_norm_file):
            shutil.copyfile(contact_norm_file, "./contact_norm.json")
            with open(contact_norm_file) as f:
                cn = json.load(f)
            self._contact_loc   = np.array(cn["loc"],   dtype=np.float32)
            self._contact_scale = np.array(cn["scale"], dtype=np.float32)
            print("[RobobufReplayBuffer] Contact anchor conditioning enabled")
        ...
        for idx in tqdm.tqdm(index_list):
            ...
            # Extract contact anchor stored in obs by the converter (if present)
    ...
    def __getitem__(self, idx):
        step, a_t, loss_mask, goal, contact_anchor_raw = self.s_a_mask[idx]
        ...
        # contact_anchor is already normalized by the converter before saving to buf.pkl
        if contact_anchor_raw is not None and self._contact_loc is not None:
            return (i_t, o_t), a_t, loss_mask, _to_tensor(contact_anchor_raw)
