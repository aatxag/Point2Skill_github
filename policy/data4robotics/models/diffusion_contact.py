...
class _DiTNoiseNet(nn.Module):
    def __init__(
        ...
        # Contact anchor goal conditioning — optional 3-D point in robot frame.
        # Added to the timestep embedding so it modulates every adaLN block in
        # the decoder.  Last layer is zero-initialised (adaLN-zero principle):
        # at init the contact has no effect, preserving pretrained weights.
        self.contact_emb = nn.Sequential(
            nn.Linear(3, hidden_dim),
            nn.SiLU(),
            nn.Linear(hidden_dim, hidden_dim),
        )
        nn.init.zeros_(self.contact_emb[-1].weight)
        nn.init.zeros_(self.contact_emb[-1].bias)
    ...
    def forward(self, noise_actions, time, obs_enc, enc_cache=None, contact_point=None):
        if enc_cache is None:
            enc_cache = self.forward_enc(obs_enc)
        return enc_cache, self.forward_dec(noise_actions, time, enc_cache, contact_point=contact_point)
    ...
    def forward_dec(self, noise_actions, time, enc_cache, contact_point=None):
        time_enc = self.time_net(time)

        # Add contact anchor embedding to the conditioning signal.
        # contact_point: (B, 3) normalised 3-D point, or None for unconditional.
        if contact_point is not None:
            time_enc = time_enc + self.contact_emb(contact_point)
...
class DiffusionTransformerAgent(BaseAgent):
    ...
    def forward(self, imgs, obs, ac_flat, mask_flat, contact_point=None):
        ...
        _, noise_pred = self.noise_net(noise_acs, timesteps, s_t, contact_point=contact_point)
    ...
    def get_actions(self, imgs, obs, n_steps=None, contact_point=None):
        ...
        for timestep in self.diffusion_schedule.timesteps:
            ...
            noise_pred = self.noise_net.forward_dec(
                noise_actions, batched_timestep, enc_cache, contact_point=contact_point
            )
