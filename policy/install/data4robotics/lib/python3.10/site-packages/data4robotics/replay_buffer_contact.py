...
class RobobufReplayBuffer(Dataset):
    def __init__(
        ...
        self._contact_mean = None
        self._contact_std = None
        if os.path.exists(contact_norm_file):
            shutil.copyfile(contact_norm_file, "./contact_norm.json")
            with open(contact_norm_file) as f:
                cn = json.load(f)
            self._contact_mean = np.array(cn["mean"], dtype=np.float32)
            self._contact_std = np.array(cn["std"], dtype=np.float32)
    ...
    def __getitem__(self, idx):
        ...
        # Return contact anchor as an extra batch element when available
        if contact_anchor_raw is not None and self._contact_mean is not None:
            ca_norm = (contact_anchor_raw - self._contact_mean) / self._contact_std
            return (i_t, o_t), a_t, loss_mask, _to_tensor(ca_norm)
