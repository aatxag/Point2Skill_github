...
Episodio bakoitzarentzat irakurri:
- Traj.npz:

q:
  joints del robot, dimensión 7.

g:
  width medido del gripper. Es el valor físico leído durante la demo.

g_cmd:
  comando lógico del gripper convertido a anchura:
    abierto -> 0.08
    cerrado -> 0.005

ee_pose_clean:
  pose del end-effector por frame:
    [x, y, z, qx, qy, qz, qw]


- Jasoten da alde batetik g eta bestetik comandoa:
Aurkitu trantsizioa, noiz agindu den istea eta noiz gelditzen den width a estable

- Behin frame a aukeratutakoan, posea irakurtzen da alde batetik
Eta kontaktu pixela ere bai, ondoren hau transformatzeko:

pixel (160,110)
  + depth del frame de contacto
  -> p_cam

p_cam
  -> T_cam_to_ee (hand-eye calibration)
  -> p_ee en el frame del end-effector del contacto

p_ee
  -> T_ee_to_base de ee_pose[frame_index]
  -> p_base

- Ondoren timestep bakoitzeko anchor-a kalkulatzen da:
P_base daukagunez eta
T_ee_grasp_to_base = pose7_to_matrix(ee_pose_arr[frame_index]) kalkulatzen da, non frame_index da kontaktuaren frame-a
i puntu bakoitzean base-ko puntua ee-ra pasatzen dugu

NORMALIZAZIOA:
Akumulatu kontaktu puntu guztiak.
1 eta 99 percentilak kalkulatu, eta horiek erabili normalizatzeko (loc, scale) moduan, berdin akzioetarako.
