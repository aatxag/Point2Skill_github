...
Equivalente a convert_to_robobuf_contact_hindsight.py pero para tareas de PLACE:
  ...
  - El frame de referencia es cuando el gripper ABRE físicamente según la señal medida.
...
de contact). Añade además:
  ...
  - detección de release por apertura física real del gripper, no por comando OPEN
"""
...
def detect_open_command_frame(cmd_trace: np.ndarray) -> int:
    """
    Devuelve el primer frame donde el comando de gripper pasa de cerrado a abierto.

    Solo se usa como punto de inicio de búsqueda: el frame de place final se
    decide con la apertura medida físicamente en g_trace, no con el comando.
    """
    cmd = np.asarray(cmd_trace).reshape(-1)
    if len(cmd) < 2:
        return -1
...
def detect_place_frame(
    g_trace: np.ndarray,
    cmd_trace: np.ndarray,
    physical_open_ratio: float = 0.25,
    physical_open_min_delta: float = 1e-4,
    physical_open_stable_frames: int = 1,
    **_,
) -> int:
    """
    Devuelve el frame donde la pinza se ha abierto físicamente según la señal
    medida del gripper (g_trace).

    Lógica:
      1) Detecta primero el frame donde se manda OPEN en cmd_trace.
      2) Calcula un valor cerrado de referencia usando g_trace justo antes del
         comando OPEN.
      3) Calcula un valor abierto de referencia con los valores altos de g_trace
         después del comando OPEN.
      4) Devuelve el primer frame posterior al comando donde g_trace ha avanzado
         physical_open_ratio del camino entre cerrado y abierto.

    Ejemplo: physical_open_ratio=0.25 significa "el primer frame donde la pinza
    ya ha hecho el 25% de la apertura física medida". Si quieres esperar a que
    esté casi abierta del todo, usa un valor como 0.8.

    Devuelve -1 si no se puede detectar una apertura física válida.
    """
    g = np.asarray(g_trace, dtype=np.float64).reshape(-1)
    if len(g) < 2 or not np.isfinite(g).any():
        return -1

    cmd_open_frame = detect_open_command_frame(cmd_trace)
    search_start = max(1, cmd_open_frame if cmd_open_frame >= 0 else 1)
    search_start = min(search_start, len(g) - 1)

    ratio = float(np.clip(physical_open_ratio, 0.0, 1.0))
    stable_frames = max(1, int(physical_open_stable_frames))

    # Baseline cerrado: preferimos los frames inmediatamente anteriores al
    # comando OPEN, porque en PLACE el episodio empieza con la pinza cerrada.
    before = g[:search_start]
    before = before[np.isfinite(before)]
    if len(before) > 0:
        close_val = float(np.median(before[-min(10, len(before)):]))
    else:
        close_val = float(np.nanpercentile(g, 5))

    # Referencia abierta: valores altos después del comando OPEN. Usamos
    # percentil 95 para evitar que un pico aislado determine el umbral.
    after = g[search_start:]
    after = after[np.isfinite(after)]
    if len(after) > 0:
        open_val = float(np.nanpercentile(after, 95))
    else:
        open_val = float(np.nanpercentile(g, 95))

    delta = open_val - close_val
    if abs(delta) < float(physical_open_min_delta):
        return -1

    # Normalmente open_val > close_val. Dejamos también el caso inverso por si
    # algún recorder codifica apertura con valores menores.
    direction = 1.0 if delta > 0 else -1.0
    threshold = close_val + ratio * delta

    last_start = len(g) - stable_frames
    for i in range(search_start, last_start + 1):
        window = g[i:i + stable_frames]
        if not np.isfinite(window).all():
            continue
        if direction > 0 and np.all(window >= threshold):
            return i
        if direction < 0 and np.all(window <= threshold):
            return i
...
def convert_dataset(
    ...
    for ep in tqdm(episode_dirs, desc="Episodes"):
        ...
        # Detectar frame de release físico: primero se localiza el comando OPEN,
        # pero el frame final se elige con la apertura medida del gripper (g).
        frame_index = detect_place_frame(
            g,
            data["gripper_cmd"][:t],
            physical_open_ratio=physical_open_ratio,
            physical_open_min_delta=physical_open_min_delta,
            physical_open_stable_frames=physical_open_stable_frames,
        )
        if frame_index < 0:
            print(f"[{ep.name}] SKIP — no se detecta apertura física del gripper")
            skipped += 1
            continue
        ...
        debug_info["physical_open_ratio"] = float(physical_open_ratio)
        debug_info["physical_open_min_delta"] = float(physical_open_min_delta)
        debug_info["physical_open_stable_frames"] = int(physical_open_stable_frames)
...
def main():
    ...
    parser.add_argument(
        ...
        default=0.25,
        help=(
            "Fracción de apertura física medida para elegir el frame de release. "
            "0.25 detecta el primer movimiento claro; 0.8 espera a que esté casi abierta."
        ),
    )
    parser.add_argument(
        "--physical_open_min_delta",
        type=float,
        default=1e-4,
        help="Cambio mínimo de gripper medido para considerar que hay apertura física.",
    )
    parser.add_argument(
        "--physical_open_stable_frames",
        type=int,
        default=1,
        help="Número de frames consecutivos que deben superar el umbral físico de apertura.",
    )
    ...
    convert_dataset(
        ...
        physical_open_ratio=args.physical_open_ratio,
        physical_open_min_delta=args.physical_open_min_delta,
        physical_open_stable_frames=args.physical_open_stable_frames,
    )
