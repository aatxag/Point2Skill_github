...
def go_to_home(env, q_home, hz=5.0, dq_max=0.05, q_thresh=0.05, timeout=30.0):
    ...
    period = 1.0 / hz
    t0     = time.time()
    print(f"\n[AUTO_LIFT] Moving to home: {np.round(q_home, 4).tolist()}")
    while True:
...
def main():
    ...
    parser.add_argument("--home_dq_max", default=0.08, type=float)
    parser.add_argument("--ready_q", nargs=7, type=float, metavar="Q",
                        default=[-0.29019999504089355, -0.5712000131607056,  0.15060000121593475,
                                 -2.533400058746338,    0.11640000343322754,  1.8801000118255615,
                                 -2.587100028991699],
                        help="Posición articular a la que ir antes del click UI.")
    ...
    for rollout_num in range(args.num_rollouts):
        ...
        # ── Move to ready position before click UI ────────────────────────────
        _ready_q = np.array(args.ready_q, dtype=np.float32)
        print(f"\n[ROLLOUT {rollout_num}] Moving to ready position...")
        go_to_home(env, _ready_q, hz=args.home_hz, dq_max=args.home_dq_max)
        ...
        # Capture start position — used as home target for auto_lift
        q_start = env.node.get_q().copy()
        print(f"[ROLLOUT {rollout_num}] Start q: {np.round(q_start, 4).tolist()}")
        print(f"[ROLLOUT {rollout_num}] Starting — {args.T} steps")

        # Coffee:  [-0.29019999504089355, -0.5712000131607056, 0.15060000121593475, -2.533400058746338, 0.11640000343322754, 1.8801000118255615, -2.587100028991699]
