...
POLICY_DESCRIPTIONS = (
    ...
    "  place_drawer : place the held object into the drawer\n"
...
class PlannerNode(Node):
    def __init__(self):
        ...
        self.declare_parameter("topic_plan_approval",   "/planner_approval")
        ...
        t_approval  = self.get_parameter("topic_plan_approval").value
        ...
        # Allow cb_policy_status / cb_plan_approval to fire while cb_prompt is blocked
        ...
        self.create_subscription(String, t_pstatus,  self.cb_policy_status,  qos,
                                 callback_group=_reentrant)
        self.create_subscription(String, t_approval, self.cb_plan_approval,  qos,
                                 callback_group=_reentrant)
        ...
        self.get_logger().info(f"[Planner] Approval topic  : {t_approval}  (confirm_plan={self.confirm_plan})")
    ...
    def cb_plan_approval(self, msg: String):
        self.plan_approval = msg.data.strip().lower()
        self.get_logger().info(f"[Planner] Operator approval: '{self.plan_approval}'")
    ...
    def _generate_plan(self, command: str, img_pil) -> list | None:
        prompt = (
            "You are a robot planner. Convert the command into a minimal JSON plan.\n"
            "Use only these actions: open, close, pick, place.\n"
            "Rules:\n"
            "- open:  use target for what must be opened, object must be null.\n"
            "- close: use target for what must be closed, object must be null.\n"
            "- pick:  use object for what must be picked, target must be null.\n"
            "- place: use object for what is held, target for where it must be placed.\n"
            "- If placing something inside a drawer, open it first.\n"
            "- If asked to close something, add a close step at the end.\n"
            "- Output JSON array only. No explanation.\n\n"
            "Example:\n"
            "Command: Put the coffee inside the drawer and close it\n"
            "["
            "{\"step\":1,\"action\":\"open\",\"object\":null,\"target\":\"drawer\"},"
            "{\"step\":2,\"action\":\"pick\",\"object\":\"coffee\",\"target\":null},"
            "{\"step\":3,\"action\":\"place\",\"object\":\"coffee\",\"target\":\"drawer\"},"
            "{\"step\":4,\"action\":\"close\",\"object\":null,\"target\":\"drawer\"}"
            "]\n\n"
            f"Command: {command}\n"
        )
    ...
    _ACCEPT_WORDS = {"accept", "accepted", "yes", "y", "approve", "approved", "ok"}
    _REJECT_WORDS = {"reject", "rejected", "no", "n", "cancel", "abort"}

    def _wait_for_approval(self, plan_json: str, timeout: float = 300.0) -> "str | None":
        """Show plan for operator review. Returns (possibly edited) JSON string or None if rejected."""
        self.plan_approval = None

        display = os.environ.get("DISPLAY", "")
        if display:
            return self._gui_approval(plan_json, timeout)

        # Headless fallback: wait for /planner_approval topic
        self.get_logger().info(
            f"[Planner] No DISPLAY — waiting for topic approval (timeout={timeout}s)\n"
            "  Accept: ros2 topic pub --once /planner_approval std_msgs/msg/String \"{data: 'accept'}\"\n"
            "  Reject: ros2 topic pub --once /planner_approval std_msgs/msg/String \"{data: 'reject'}\""
        )
        t0 = time.time()
        while rclpy.ok():
            v = self.plan_approval
            if v in self._ACCEPT_WORDS:
                self.get_logger().info("[Planner] Plan APPROVED via topic.")
                return plan_json
            if v in self._REJECT_WORDS:
                self.get_logger().warning("[Planner] Plan REJECTED via topic.")
                return None
            if time.time() - t0 > timeout:
                self.get_logger().error(f"[Planner] Approval timeout after {timeout}s.")
                return None
            time.sleep(0.1)
        return None

    def _gui_approval(self, plan_json: str, timeout: float = 300.0) -> "str | None":
        """Open a tkinter window with editable plan JSON. Returns edited JSON or None if rejected."""
        import tkinter as tk

        result = [None]   # will hold the final JSON string if approved
        done = threading.Event()

        def _run():
            try:
                root = tk.Tk()
                root.title("Plan Confirmation")
                ...
                tk.Label(
                    root,
                    text="Review & edit the plan, then approve or reject",
                    bg="#2b2b2b", fg="#aaaaaa",
                    font=("Helvetica", 11),
                    pady=8,
                ).pack(fill="x")
                ...
                # ── editable JSON area ────────────────────────────────────────
                frame = tk.Frame(root, bg="#1e1e1e", bd=1, relief="sunken")
                ...
                def _approve():
                    ...
                    try:
                        ...
                        err_var.set(f"⚠  Invalid JSON: {exc}")
                        return
                    result[0] = edited
                    root.destroy()
                ...
                tk.Button(
                    ...
                    command=_approve,
                    bg="#2d6a2d", fg="white", activebackground="#3a8a3a",
                ...
                tk.Button(
                    ...
                    command=_reject,
                    bg="#6a2d2d", fg="white", activebackground="#8a3a3a",
                    font=("Helvetica", 12, "bold"),
                    relief="flat", padx=20, pady=8, cursor="hand2",
                ).pack(side="left", expand=True, fill="x", padx=(6, 0))

                root.protocol("WM_DELETE_WINDOW", _reject)
                root.lift()
                root.attributes("-topmost", True)
                root.mainloop()
            except Exception as exc:
                self.get_logger().error(f"[Planner] GUI error: {exc}")
            finally:
                done.set()

        t = threading.Thread(target=_run, daemon=True)
        t.start()
        self.get_logger().info("[Planner] Waiting for operator decision in GUI window…")
        done.wait(timeout=timeout)

        if result[0] is not None:
            self.get_logger().info("[Planner] Plan APPROVED via GUI.")
            return result[0]
        self.get_logger().warning("[Planner] Plan REJECTED via GUI (or window closed).")
        return None
    ...
    def run_plan(self):
        ...
        plan = self._generate_plan(self.last_prompt, img_pil)

        if not plan:
            self._pub_response(f"Planning failed for: '{self.last_prompt}'")
            return

        # ── 2. Operator approval — editable JSON before policy mapping ────────
        if self.confirm_plan:
            plan_json = json.dumps(plan, indent=2, ensure_ascii=False)
            edited_json = self._wait_for_approval(plan_json)
            if edited_json is None:
                self._pub_response("Plan rejected or approval timeout — execution aborted.")
                return
            try:
                plan = json.loads(edited_json)
                if not isinstance(plan, list):
                    plan = [plan]
            except json.JSONDecodeError as exc:
                self._pub_response(f"Invalid JSON after editing: {exc}")
                return
